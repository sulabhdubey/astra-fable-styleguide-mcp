...
import { deepClone, isRecord } from '../../style-spec/src/index.js';
...
function normalizeProposal(value:unknown,role:string,baseVersion:string,previousId?:string):DesignProposal{
  ...
  const changes=rawChanges.map((change,i)=>{if(!isRecord(change))throw new Error(`Invalid change at ${i}`);return {path:assertString(change.path,`changes[${i}].path`),value:deepClone(change.value),...(typeof change.rationale==='string'?{rationale:change.rationale}:{})};});
...
function normalizeCritique(value:unknown,role:string,proposalId:string):Critique{
  ...
  return {reviewer:role,proposalId,objections,acceptedPaths};
}
...
export class ConfigurableAgent implements DesignAgent {
  ...
  async critiqueProposal(proposal:Readonly<DesignProposal>,context:Readonly<DesignContext>):Promise<Critique>{return normalizeCritique(await this.invoke({role:this.id,model:this.config.model,task:'critique',payload:{proposal,context}}),this.id,proposal.id);}
...
function taskPrompt(request:{role:string;model:string;task:AgentTask;payload:unknown}):string{
  ...
  if(request.task==='critique')return shared+`TASK: Critique the counterpart proposal against the product brief, criteria, and referenceSpec. JSON shape: {"objections":[{"path":"...","reason":"...","severity":"blocking|warning"}],"acceptedPaths":["..."]}. Blocking objections should explain a concrete correction or measurable concern. INPUT: ${JSON.stringify(request.payload)}`;
