...
Authenticated proposal rounds may add a complete token leaf under an existing token group and a component token mapping under an existing component's `tokens` object. They may also append accessibility rules or contrast pairs, append rules to an existing pattern, append component rule IDs or `contentRules`/`do`/`dont` guidance, and add `errorTextRequired` or `errorAssociation` to an existing component's accessibility object. Existing entries in these lists must stay intact and in order. Contrast token paths, component rule references, and entry shapes are checked deterministically. The merged candidate must pass the full validator before it receives a hash. Candidate creation does not edit `/spec`; canonical changes require a separate reviewed commit. The optional local provider adapter exposes the accessibility and pattern paths, but it still constrains token additions to the authenticated proposal workflow.
