...
v0.4.0. The browser documentation and `/version` are human checks; opening `/mcp`
...
model performance. Demo setup commands require this onboarding revision; the
v0.4.0 tag contains the underlying verifier and repair APIs, not the standalone CLI.
...
`npm install /absolute/path/styleconstitution-cli-0.4.0.tgz` and use
