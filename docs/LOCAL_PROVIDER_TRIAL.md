...
## Open-brief evaluator finding, 2026-09-19

A separate local run on commit `2f9f2c07255b924b1d535d12c1a844e425c99b8e` asked two distinct models for a focused settings-form improvement without prescribing a path or value. The then-current evaluator reported consensus for a candidate containing `tokens.elevation.md.$value = "'lg'"` and a changed red token. The shadow name is not a CSS shadow. One model also claimed the new red had 4.5:1 contrast on white, while deterministic contrast calculation gives approximately 4.13:1. The canonical accessibility rules do not currently declare that danger-text contrast pair, so this claim was outside the evaluator's coverage. The candidate was rejected as design evidence; `/spec` and the release were unchanged.

The shadow token grammar and unchanged-value candidate checks were tightened after this observation. Replaying the recorded candidate against the updated evaluator rejects it with `STYLE-TOKEN-004`. An explicit danger-text contrast pair remains a proposed v0.2.0 design change, subject to the governed canonical-spec review.
