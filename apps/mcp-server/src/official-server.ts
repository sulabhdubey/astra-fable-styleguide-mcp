...
function buildServer(){
 ...
 server.registerTool('get_style_manifest',{annotations:{readOnlyHint:true,destructiveHint:false,idempotentHint:true,openWorldHint:false},description:'Return the canonical style manifest'},async()=>asText(service.getManifest()));
 server.registerTool('get_design_tokens',{annotations:{readOnlyHint:true,destructiveHint:false,idempotentHint:true,openWorldHint:false},description:'Return all tokens or one token scope',inputSchema:z.object({scope:z.string().optional()})},async({scope})=>asText(service.getDesignTokens(scope)));
 server.registerTool('get_component_rules',{annotations:{readOnlyHint:true,destructiveHint:false,idempotentHint:true,openWorldHint:false},description:'Return canonical rules for a component',inputSchema:z.object({component:z.string()})},async({component})=>asText(service.getComponentRules(component)));
 server.registerTool('search_style_spec',{annotations:{readOnlyHint:true,destructiveHint:false,idempotentHint:true,openWorldHint:false},description:'Search components and decision records',inputSchema:z.object({query:z.string().min(1)})},async({query})=>asText(service.search(query)));
 server.registerTool('explain_style_decision',{annotations:{readOnlyHint:true,destructiveHint:false,idempotentHint:true,openWorldHint:false},description:'Explain an ADR by id/filename',inputSchema:z.object({id:z.string()})},async({id})=>asText(service.explainDecision(id)));
 server.registerTool('validate_tokens',{annotations:{readOnlyHint:true,destructiveHint:false,idempotentHint:true,openWorldHint:false},description:'Run deterministic validation of the canonical StyleSpec'},async()=>asText(service.validate()));
 server.registerTool('check_style_compliance',{annotations:{readOnlyHint:true,destructiveHint:false,idempotentHint:true,openWorldHint:false},description:'Check CSS color/pixel literals and canonical semantic token references; returns source locations and explicit coverage limits',inputSchema:z.object({input:z.string().max(200000)})},async({input})=>asText(service.checkStyleCompliance(input)));
 server.registerTool('compare_spec_versions',{annotations:{readOnlyHint:true,destructiveHint:false,idempotentHint:true,openWorldHint:false},description:'Compare known immutable StyleSpec snapshots',inputSchema:z.object({fromVersion:z.string(),toVersion:z.string()})},async({fromVersion,toVersion})=>asText(service.compareSpecVersions(fromVersion,toVersion)));
