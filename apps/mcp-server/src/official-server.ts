...
function buildServer(){
 ...
 server.registerTool('check_style_compliance',{description:'Check supplied UI/code text for obvious token/style violations',inputSchema:z.object({input:z.string().max(200000)})},async({input})=>asText(service.checkStyleCompliance(input)));
 ...
 if(process.env.MCP_ENABLE_WRITES==='true'){
   ...
   server.registerTool('get_consensus_status',{description:'Return candidate status',inputSchema:z.object({candidateHash:z.string()})},async({candidateHash})=>asText(service.getConsensusStatus(candidateHash)));
